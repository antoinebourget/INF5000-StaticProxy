...
public class Lower extends TreeTranslator {
     ...
     *
     * <p>For each enum that appears as the type of a switch expression, we
     ...
     *          case 2: stmt2
     *          }
     * </pre> with the auxiliary table initialized as follows:
     * <pre>
     *          class Outer$0 {
     ...
     *              }
     *          }
     * </pre> class EnumMapping provides mapping data and support methods for
     * this translation.
     */
    class EnumMapping {
        ...
        EnumMapping(DiagnosticPosition pos, TypeSymbol forEnum) {
            ...
            Name varName = names.fromString(target.syntheticNameChar()
                    ...
                    + writer.xClassName(forEnum.type).toString().replace('/', '.').replace('.', target.syntheticNameChar()));
        ...
        void translate() {
            ...
            JCExpression size = make // Color.values().length
                    ...
                    syms.lengthVar);
            JCExpression mapVarInit = make.NewArray(make.Type(syms.intType), List.of(size), null).setType(new ArrayType(syms.intType, syms.arrayClass));
            ...
            Symbol ordinalMethod = lookupMethod(pos,
                    ...
                    forEnum.type,
                    List.<Type>nil());
            List<JCCatch> catcher = List.<JCCatch>nil().prepend(make.Catch(make.VarDef(new VarSymbol(PARAMETER, names.ex,
            ...
            for (Map.Entry<VarSymbol, Integer> e : values.entrySet()) {
                ...
                JCExpression assign = make.Assign(make.Indexed(mapVar,
            ...
            owner.defs = owner.defs.prepend(make.Block(STATIC, stmts.toList())).prepend(make.VarDef(mapVar, mapVarInit));
        }
    }
     ...
     * @param enclOp The closest enclosing operation node of tree, null if tree
     * is not a subtree of an operation.
     */
...
//      System.err.println("access " + vsym + " with " + accessor);//DEBUG
    ...
    Scope proxies;
    /**
     * A scope containing all unnamed resource variables/saved exception
     * variables for translated TWR blocks
     */
    Scope twrVars;
    /**
...
//where
     ...
     * @return A a desugared try-with-resources tree, or the original try block
     * if there are no resources to manage.
     */
     ...
     * @param pos The source code position to be used for the tree.
     * @param sym The accessed symbol.
     * @param preciseMatch should we accept a type that is a subtype of sym's
     * owner, even if it doesn't contain sym due to hiding, overriding, or
        ...
        switch (type.tag) {
            ...
            case ARRAY:
                ...
                return make_at(pos).Conditional(
                        ...
                        List.<JCExpression>of(make.Literal(CLASS, sig).setType(syms.stringType)))).setType(types.erasure(syms.classType)),
     ...
     * </pre> where
     * <code><b>TEMP</b></code> is a newly declared variable in the let
     * expression.
     */
    ...
    public void visitTopLevel(JCCompilationUnit tree) {
        if (needPackageInfoClass(tree)) {
            ...
            if (target.isPackageInfoSynthetic()) // package-info is marked SYNTHETIC in JDK 1.6 and later releases
            {
                flags = flags | Flags.SYNTHETIC;
            }
            JCClassDecl packageAnnotationsClass = make.ClassDef(make.Modifiers(flags,
    ...
    public void visitMethodDef(JCMethodDecl tree) {
        if (tree.name == names.init && (currentClass.flags_field & ENUM) != 0) {
            ...
            JCVariableDecl ordParam = make.Param(names.fromString(target.syntheticNameChar()
...
//where
    ...
    private void visitArrayForeachLoop(JCEnhancedForLoop tree) {
        ...
        result = translate(make.ForLoop(loopinit,
                cond,
                List.of(step),
                body));
    ...
    private void visitIterableForeachLoop(JCEnhancedForLoop tree) {
        ...
        result = translate(make.ForLoop(List.of(init),
                cond,
    ...
    public JCTree visitStringSwitch(JCSwitch tree) {
        ...
        if (alternatives == 0) { // Strange but legal possibility
             ...
             * statement into a series of two chained switch statements: the
             * first a synthesized statement switching on the argument string's
             * hash value and computing a string's position in the list of
             * original case labels, if any, followed by a second switch on the
             * computed integer value. The second switch has the same code
             * structure as the original string switch statement except that the
             * string case labels are replaced with positional integer constants
             * starting at 0.
             *
             * The first switch statement can be thought of as an inlined map
             * from strings to their position in the case label list. An
             * alternate implementation would use an actual Map for this
             * purpose, as done for enum switches.
             *
             * With some additional effort, it would be possible to use a single
             * switch statement on the hash code of the argument, but care would
             * need to be taken to preserve the proper control flow in the
             * presence of hash collisions and other complications, such as
             * fallthroughs. Switch statements with one or two alternatives
             * could also be specially translated into if-then statements to
             * omit the computation of the hash code.
             *
             * The generated code assumes that the hashing algorithm of String
             * is the same in the compilation environment as in the environment
             * the code will run in. The string hashing algorithm in the SE JDK
             * has been unchanged since at least JDK 1.2. Since the algorithm
             * has been specified since that release as well, it is very
             * unlikely to be changed in the future.
             *
             * Different hashing algorithms, such as the length of the strings
             * or a perfect hashing algorithm over the particular set of case
             * labels, could potentially be used instead of String.hashCode.
             */
             ...
             * caseLabelToPosition("caseLabel_1"); else if
             * (s$.equals("caseLabel_2")) tmp$ =
             * caseLabelToPosition("caseLabel_2"); ... break; ... }
             */
            ...
            for (JCCase oneCase : caseList) {
                ...
                if (isDefault) {
                    ...
                    caseExpr = make.Literal(caseLabelToPosition.get((String) oneCase.getExpression().type.constValue()));
                }
