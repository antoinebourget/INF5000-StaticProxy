...
public class Attr extends JCTree.Visitor {
    ...
    /**
     * Switch: generates a warning if diamond can be safely applied to a given
     * new expression
     */
            ...
            if (tree.body == null) {
                ...
                if (tree.name == names.init && owner.type != syms.objectType) {
                    ...
                    if (body.stats.isEmpty()
                        ...
                        body.stats = body.stats.prepend(memberEnter.SuperCall(make.at(body.pos),
                                List.<Type>nil(),
     ...
     * that the original scope contains a constructor of the kind: Foo(X x, Y
     * y), where X,Y are class type-variables declared in Foo, the synthetic
     * scope is added a generic constructor of the kind: <X,Y>Foo<X,Y>(X x, Y
     * y). This is crucial in order to enable diamond inference. The inferred
     * return type of the synthetic constructor IS the inferred type for the
     * diamond operator.
     */
...
//                             Resolve.isStatic(env));//DEBUG
     ...
     * compiler has encountered some errors (which might have ended up
